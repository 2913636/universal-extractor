...
环境变量（至少配一个）：
    OPENAI_API_KEY    — 用于 GPT-4o 视觉 OCR
    ANTHROPIC_API_KEY — 用于 Claude Sonnet 视觉 OCR
    DASHSCOPE_API_KEY — 用于 Qwen-VL 视觉 OCR
    DEEPSEEK_API_KEY  — 用于 DeepSeek（自动探测 vision 支持）
"""
...
from universal_extractor import auto_configure_providers
...
print(f"Vision providers: {[p.name for p in providers if p.can_handle()]}")
print("=" * 60)
...
ue = UniversalExtractor(
    ...
    screenshot_overlap=0.2,  # 截帧 20% 重叠
)
